...
- Invitation sections for location, the 14-16 May 2027 weekend, travel/stay, optional vyshyvanka-inspired attire or standard wedding attire, and RSVP
...
- Superseded wedding forms were moved to Google Drive trash through the allowlisted cleanup tool; the three form IDs listed above are the active forms
...
- The Google Drive API is enabled only for guarded cleanup of obsolete project-created wedding forms; the script relies on the restricted `drive.file` scope and that scope should not be broadened.
- Run `cleanup_google_forms.py` without `--trash` before any cleanup operation.
